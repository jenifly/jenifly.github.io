<template>
  <div class="notebook">
    <div class="tb" :class="r?'tb-a':''">
      <svg v-for="(i,j) in tbs" :class="tb==j?'active':''" viewBox="0 0 1024 1024" @click="tb=j" v-html='svg[i]'/>
    </div>
    <aside ref="aside" class="notebook" :class="r?'aside-a':''">
      <svg v-show="r==1" class="left" viewBox="0 0 1024 1024" v-html="svg.r" @click="r=0"></svg>
      ...
      <ul ref="catalogue" class="ctlg" v-show="tb==1"></ul>
    </aside>
...
export default {
  data () {
    return {
      ...
      }], title: '', content: '', init: false, r: 0, k: 1, yiyan: {src: 'http://imglf5.nosdn0.126.net/img/SFh0dTdRWDlwUTIyNGk5QklpZmhXdnJhaU1GaWMxcURQZTB0MnJlM21qazFFYW9va2xIY25RPT0.jpg?imageView&thumbnail=500x0&quality=96&stripmeta=0&type=jpg', content: '五月将尽<br>连日强光普照<br>一路一树荫<br>呆滞到傍晚<br>红胸鸟在电线上啭鸣<br>天色舒齐地暗下来<br>那是慢慢地，很慢...', title: '木心《杰克逊高地》', class: '0'}, loading: false, cacheLi: undefined, heads: [], tbs: ['ku', 'catalogue' ], tb: 0
    }
  },
  created () {
    // this.getList()
  ...
  methods: {
    ...
    initMarked () {
      ...
      rendererMD.heading = (text, level, raw) => {
        this.heads.push([text, level])
        return `<h${level} id=t${this.heads.length-1}>${text}</h${level}>\n`
      }
      marked.setOptions({
        renderer: rendererMD,
        gfm: true,
    ...
    click (i, j) {
      if(i.download_url) {
        ...
        this.$set(i, 'o', !i.o)
        // if(!i.child) this.doGet(this.baseUrl + i.path, res => {
        ...
        if(this.k) {
          ...
          this.cookie.set('aside', this.$refs.aside.innerHTML, 10)
          return this.insertAfter(a, j)
        }
    ...
    catalogue () {
      for(let a=0;a<this.heads.length;a++){
        ...
        this.$refs.catalogue.appendChild(b)
      }
    },
    clg (i, j) {},
    insertAfter (e, t) {
